...
survey_expressions <- survey_expressions %>% rename(lc1=2, lc2=3, lc3=4, lc4=5, lc5=6,
                                                    lc6=7, lc7=8, lc8=9)
by_survey <- survey_expressions %>% pivot_longer(cols = !survey, names_to = "LC", values_to = "xp")
by_survey <- by_survey %>% filter(!is.na(xp)) %>% filter(xp>0)
surveys_in_plot <- by_survey %>% select(survey) %>% distinct()
surveys_in_plot <- surveys_in_plot %>% mutate(id = seq_along(survey))
number_of_bar <- length(surveys_in_plot$survey)
label_y <- by_survey %>% select(-LC) %>% group_by(survey) %>% summarise(y = sum(xp))
surveys_in_plot <- surveys_in_plot %>% mutate(y=label_y$y)
surveys_in_plot <- surveys_in_plot %>% mutate(angle =  90 - 360 * (surveys_in_plot$id-0.5) /number_of_bar)    
...
surveys_in_plot <- surveys_in_plot %>% mutate(hjust = ifelse( angle < -90, 1, 0))
# flip angle BY to make them readable
surveys_in_plot <- surveys_in_plot %>% mutate(angle=ifelse(angle < -90, angle+180, angle))
ggplot(surveys_in_plot) + 
  geom_col(data = by_survey, aes(x = survey, y = xp, fill = LC)) +
  coord_polar(start = 0) +
  ylim(-100,120) +
  # Add the labels, using the label_data dataframe that we have created before
  geom_text(data=surveys_in_plot, aes(x=id, y=y, label=survey, hjust=hjust), 
