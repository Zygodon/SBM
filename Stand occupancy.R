...
GetTheData <-  function()
{
  ...
  q <- sprintf('select survey_id, assembly_name, quadrat_count, community, quadrat_id, quadrat_size, visit_date, records_id, species.species_id, 
    species.species_name from surveys
      ...
      # Two surveys have 0 quadrat count; exclude A.capillaris_stolonifera;
      # exclude some odd surveys with no assigned community
...
d2 <- (the_data %>% select(survey_id, species_name)
...
d3 <- (d2 %>% select(-survey_id) %>% replace(., !is.na(.), 1)
...
cn <- colnames(d3)
d3 <- (mutate(d3, stand = paste("S", as.character(d2$survey_id), sep = "_"))
...
for (i in seq_along(row.names(edges)))
{
  ...
  jc <- (site_occ %>% select(e1, e2) # Two vectors, 108 long
        %>% rename(A = 1, B = 2))
    t <- table(jc$A, jc$B) # Contingency table
    if((0 %in% t) | sum(dim(t)) < 4){ # Set NAs for cases where there are no edges
      edges$p_val[i] <- NA
      edges$or[i] <- NA
      edges$lor[i] <- NA
    }else { # The contingency table is the basis for an exact test of independence
      ...
      edges$or[i] <- ft$estimate # The Fisher test generates an odds ratio.
      edges$lor[i] <- log10(ft$estimate)
    }
