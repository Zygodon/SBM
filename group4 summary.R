...
ggrp4 <- g1 %>% activate(nodes) %>% filter(group == 4)

ggrp4 %>% ggraph(layout = "kk") +
...
group4 <- ggrp4 %>% activate(nodes) %>% as_tibble

# Remove the species that are not represented in the SBM model
# i.e. not group1 species
edge_list <- survey_data %>% 
  filter(species %in% group4$name) %>% 
  filter(!is.na(community)) %>%
  select(-community)

# bp4: bipartite for group 4
bp4 <- graph.data.frame(edge_list, directed = F)
V(bp4)$type <- V(bp4)$name %in% edge_list$species #the second column of edges is TRUE type
bp4 <- as_tbl_graph(bp4)
bp4 <- bp4 %>% activate(nodes) %>% mutate(kind = ifelse(type, "species", "survey"))

# Group 4 membership weight for each survey.
# The sum of the lor for the group1 species that are represented in a survey
# surveys is a place holder for the results
surveys <- bp4 %>% activate(nodes) %>% filter(type == FALSE) %>% as_tibble()
for (i in seq_along(surveys$name)) {
  ...
  grp4_weight <- ggrp4 %>% activate(nodes) %>%
    filter(name %in% survey$name[which(survey$type == TRUE)]) %>%
    activate(edges) %>% 
    as_tibble() %>% 
    select(lor) %>% 
    summarise(sum = sum(lor))
  surveys$grp4_weight[i] <- unlist(grp4_weight)
}

surveys <- surveys %>% select(name, grp4_weight)
# Transfer the weights to the bipartite graph nodes.
bp4 <- bp4 %>% activate(nodes) %>% left_join(surveys, join_by(name))

bp4 %>% ggraph(layout = "stress") +
  geom_edge_link(colour = "grey80") +
  scale_colour_brewer(palette = "Dark2") +
  geom_node_point(aes(shape = kind, colour = kind, size = ifelse(kind == "survey", grp4_weight, 2))) +
