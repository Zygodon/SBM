...
ggrp2 <- g1 %>% activate(nodes) %>% filter(group == 2)

ggrp2 %>% ggraph(layout = "kk") +
...
group2 <- ggrp2 %>% activate(nodes) %>% as_tibble

# Remove the species that are not represented in the SBM model
# i.e. not group1 species
edge_list <- survey_data %>% 
  filter(species %in% group2$name) %>% 
  filter(!is.na(community)) %>%
  select(-community)

# bp2: bipartite for group 2
bp2 <- graph.data.frame(edge_list, directed = F)
V(bp2)$type <- V(bp2)$name %in% edge_list$species #the second column of edges is TRUE type
bp2 <- as_tbl_graph(bp2)
bp2 <- bp2 %>% activate(nodes) %>% mutate(kind = ifelse(type, "species", "survey"))

# Group 2 membership weight for each survey.
# The sum of the lor for the group1 species that are represented in a survey
# surveys is a place holder for the results
surveys <- bp2 %>% activate(nodes) %>% filter(type == FALSE) %>% as_tibble()
for (i in seq_along(surveys$name)) {
  ...
  grp2_weight <- ggrp2 %>% activate(nodes) %>%
    filter(name %in% survey$name[which(survey$type == TRUE)]) %>%
    activate(edges) %>% 
    as_tibble() %>% 
    select(lor) %>% 
    summarise(sum = sum(lor))
  surveys$grp2_weight[i] <- unlist(grp2_weight)
}

surveys <- surveys %>% select(name, grp2_weight)
# Transfer the weights to the bipartite graph nodes.
bp2 <- bp2 %>% activate(nodes) %>% left_join(surveys, join_by(name))

bp2 %>% ggraph(layout = "stress") +
  geom_edge_link(colour = "grey80") +
  scale_colour_brewer(palette = "Dark2") +
  geom_node_point(aes(shape = kind, colour = kind, size = ifelse(kind == "survey", grp2_weight, 2))) +
